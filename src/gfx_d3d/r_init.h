...
#define MAX_SORTED_MATERIALS 4096

struct GfxWorld;
struct __declspec(align(128)) r_global_permanent_t // retail sizeof=0x2180; sortedMaterials is larger here
{                                       // ...
    // Material_Sort snapshots the whole material pool into this array and hands
    // out materialSortedIndex from it, so any material the array cannot hold
    // never gets a sort index at all. It has to track POOLSIZE_MATERIAL, which
    // db_registry.cpp static_asserts.
    Material *sortedMaterials[MAX_SORTED_MATERIALS];
